...
x_list = [0, 2, 7]  # Tạo danh sách các giá trị của trục x
y_list = [1, 8, 9]  # Tạo danh sách các giá trị tương ứng của trục y
plt.plot(x_list, y_list)  # Vẽ đồ thị với các điểm (x, y)
...
y_1 = f(x_1)  # Tính giá trị y tương ứng với mỗi giá trị x từ hàm f(x)
plt.plot(x_1, y_1)  # Vẽ đồ thị hàm số
...
plt.plot(x_1, y_1)  # Vẽ lại đồ thị với nhãn và giới hạn mới
plt.show()  # Hiển thị đồ thị
...
height = np.array([167, 170, 149, 165, 155, 180, 166, 146, 159, 185, 145, 168, 172, 181, 169])  # Dữ liệu chiều cao
weight = np.array([86, 74, 66, 78, 68, 79, 90, 73, 70, 88, 66, 84, 67, 84, 77])  # Dữ liệu cân nặng

colors = np.random.rand(15)  # Tạo mảng ngẫu nhiên các giá trị màu sắc
area = (30 * np.random.rand(15))**2  # Tạo mảng kích thước ngẫu nhiên cho các điểm

plt.xlim(140, 200)  # Giới hạn trục x từ 140 đến 200
plt.ylim(60, 100)  # Giới hạn trục y từ 60 đến 100
plt.scatter(height, weight, s=area, c=colors)  # Vẽ đồ thị phân tán với các điểm có kích thước và màu sắc ngẫu nhiên
...
h = np.array([167, 170, 149, 165, 155, 180, 166, 146, 159, 185, 145, 168, 172, 181, 189])  # Dữ liệu chiều cao 3D
w = np.array([86, 74, 66, 78, 68, 79, 90, 73, 70, 88, 66, 84, 67, 84, 77])  # Dữ liệu cân nặng 3D

# Tạo đồ thị 3D
ax = plt.axes(projection='3d')  # Tạo đối tượng axes cho đồ thị 3D
ax.scatter3D(h, w)  # Vẽ các điểm phân tán trong không gian 3D
ax.set_xlabel("Chiều cao")  # Nhãn trục x
ax.set_ylabel("Cân nặng")  # Nhãn trục y
