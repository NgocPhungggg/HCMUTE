...
namespace User_Management
{
    public partial class formPrivileges : Form
    {
        ...
        {
            string role_user = textBox_RoleUser.Text.Trim();
            string system_privileges = textBox_System.Text.Trim();
            bool option = false;
        ...
        {
            string role_user = textBox_RoleUser.Text.Trim();
            string object_name = textBox_Object.Text.Trim();
            bool option = false;
            if (radioButton.Checked == true)
            {
                option = true;
            ...
            else
            {
                ...
                {
                    string err = string.Empty;
                    bool result = privileges.GantPrivileges_Object(ref err, privilege, role_user, object_name, option);
                    ShowMessage(result, err);
                    if (!result)
                    {
                        break;
        ...
        private void button_ObjectRevoke_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    string err = string.Empty;
                    bool result = privileges.RevokePrivileges_Object(ref err, privilege, role_user, object_name);
                    ShowMessage(result, err);
                    if (!result)
                    {
                        break;
